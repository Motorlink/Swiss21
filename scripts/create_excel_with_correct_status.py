...
# Status-Farben
status_offen_fill = PatternFill(start_color="FFC7CE", end_color="FFC7CE", fill_type="solid")  # Rot
status_bezahlt_fill = PatternFill(start_color="C6EFCE", end_color="C6EFCE", fill_type="solid")  # Grün
...
# Für jeden Kunden mit Rechnungen ein Sheet erstellen
for kunde_nr in sorted(rechnungen_pro_kunde.keys()):
    ...
    # Tabellen-Header
    headers = ['Rechnung', 'Datum', 'Fällig', 'Betrag (CHF)', 'Bezahlt am', 'Status', 'Skonto %', 'Skonto Tage']
    ...
    for rechnung in sorted(rechnungen_pro_kunde[kunde_nr], key=lambda x: x.get('datum', '')):
        ws[f'A{row}'] = rechnung.get('rechnungsnummer', '')
        ws[f'B{row}'] = rechnung.get('datum', '')
        ws[f'C{row}'] = rechnung.get('faelligkeitsdatum', '')
        ws[f'D{row}'] = float(rechnung.get('betrag_brutto', 0))
        # Zahlungsstatus: komplett_bezahlt = True bedeutet bezahlt
        ist_bezahlt = rechnung.get('komplett_bezahlt', False)
        ws[f'E{row}'] = rechnung.get('faelligkeit', '') if ist_bezahlt else ''
        
        # STATUS mit Farbe
        status = 'BEZAHLT' if ist_bezahlt else 'OFFEN'
        ws[f'F{row}'] = status
        ws[f'F{row}'].font = Font(bold=True, size=10)
        ws[f'F{row}'].alignment = Alignment(horizontal='center')
        
        if status == 'OFFEN':
            ws[f'F{row}'].fill = status_offen_fill
        else:
            ws[f'F{row}'].fill = status_bezahlt_fill
        
        ws[f'G{row}'] = rechnung.get('skonto_prozent', '')
        ws[f'H{row}'] = rechnung.get('skonto_tage', '')
...
output_path = '/home/ubuntu/Swiss21/data/Kunden_Rechnungen_Korrekt.xlsx'
wb.save(output_path)
...
print(f"✓ Zahlungsstatus korrekt aus Datenbank übernommen")
print("=" * 80)
